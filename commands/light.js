...
module.exports = {
	...
	execute(client, message, args, functions) {
		if (args[0] === 'on') {
			...
			message.channel.send(functions.simpleEmbed('Light toggled on!', '', '0xffff00'));
			return;
		}
		if (args[0] === 'off') {
			mclient.publish('cmnd/yami/Power0', 'OFF');
			message.channel.send(functions.simpleEmbed('Light toggled off!', '', '0x808080'));
			return;
		}
		if (args[0] === 'color') {
			mclient.publish('cmnd/yami/Color1', args[1]);
			message.channel.send(functions.simpleEmbed('Color changed!', '', args[1]));
			return;
		}
	},
};